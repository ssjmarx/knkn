...
## Session update (2026-09-10 — the move-table extension's test sync, agent-applied under override)

- **The human extended `src/data/moves.ts` 24 → 37 rows** (strike/fang-barrage/true-strike, the quickstep/fade-step/ghost-step step family, the attune and curl progressions, dawn-bolt — per the consolidated math-update plan) and updated the two new-field pins (foeStages, pen/accuracy) but left the three whole-table pins stale; `npm run verify` failed 3/86, all `toEqual` suffix diffs.
- **Diagnosis:** clean-suffix diff = intentional table growth + unfinished transcription, not a regression. tsc can't catch stale pins (a 37-row table satisfies the test's types); only runtime fails. The count audit + the two passing new-field tests proved the additions were deliberate.
- **Fix (human-issued explicit `.clinerules` override for this session):** agent re-transcribed the three pins in `tests/moves.test.ts` (id list 37, power/weight triples, selfStages payloads incl. fade-step/ghost-step appearing in both stage pins), retitled the count pin "exactly the 37", synced the "shared 24" header/docstring in both files. **`npm run verify` green: 9 files / 86 tests, map regenerated (25 modules), build clean** (1.39 MB chunk warning = known filed item).

...
9. `memory-bank/archive/` — the long-term verbatim archive (retired session updates, old decision-log rows, engine/test lesson notes, the retired instructions copy, the generated project map). On-demand reference — consult it when a living file points there, not as standing reading. (`memory-bank/projectMap.md` may appear at the bank root: it is GENERATED by `npm run map` — never hand-edit, read on demand; the current copy lives at `archive/projectMap.md` until the generator is re-pointed.)
...
- **Retention (ruled 2026-09-05, amended 2026-09-10 — the bank compression):** session updates keep detail for the **current unit** only; at each unit boundary the previous unit's updates compress to one-liners in `activeContext.md` and their verbatim text moves to `archive/activeContext-history.md` (`progress` Done entries likewise compress to one-liners). The decision log is **never deleted or reworded** — rows older than the last two units migrate verbatim to dated files in `archive/` at unit boundaries. Reference and generated material (the project map, retired lesson notes, the old instructions copy) lives in `archive/`, read on demand. The context and canon files (brief, product context, patterns, tech, design, syllabus) are reference, not history — keep them clean and concise; they change only by human ruling. Run a compression pass at every unit completion; deep history lives in git.

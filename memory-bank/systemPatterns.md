...
- **Foxfire Split** (Mercury): multi-actor boards — 1→2→3 bodies, split HP pool, per-body Dwindle-50, per-body stat tracking. Hardest system in the game, deliberately scheduled late (Unit 14).
    Rule of 500 as a tested invariant (realized U6, evolved): the classic mirror (≈ 500 ÷ Power) is pinned as an anchor test, and the invariant generalizes into the matchup-envelope sweep — every roster pair × both channels × the level/power grids, bounded by ruled constants (hits ≥ 2 everywhere; ≤ 14 best-channel at Power ≥ 60). New spirit types append to the sweep's roster and are tripwired automatically.- **Time-driven world:** 7-day planetary calendar (each planet's day weights spawns, surges shrines, shifts schedules), festival calendar, tiered NPC schedules (3–5 featured per town; cities run shifts; Aldebaran is 24-hour). In-game time only, seeded once at file creation; no time-travel flags — events are events.
...
-     Impurity is injected: the core declares effect contracts and the edges implement them — tintForHour(hour) for time, InputSource for input, Die for randomness. The math stays pure and worst-case-testable; effects live at the composition root.
